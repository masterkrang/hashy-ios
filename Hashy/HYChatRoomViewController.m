...
@interface HYChatRoomViewController ()

@end

@implementation HYChatRoomViewController
@synthesize messages;
@synthesize subscribersCountString;
@synthesize chatNameString;
@synthesize chatDict;
@synthesize chatRoomCollectionView;
    ...
    [[NetworkEngine sharedNetworkEngine]getChatForChatRoom:^(id object) {
        ...
        [[NetworkEngine sharedNetworkEngine]getChatMessagesForChatRoom:^(id object) {
            ...
            if (![object isEqual:[NSNull null]] && [object isKindOfClass:[NSArray class]]) {
                ...
                self.messages=[object mutableCopy];
...
-(void)hashTagButtonPressed:(UIButton *)sender {
...
- (UIImageView *)collectionView:(JSQMessagesCollectionView *)collectionView bubbleImageViewForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.messages.count>indexPath.item) {
        ...
        NSMutableDictionary *messageDict=[self.messages objectAtIndex:indexPath.item];
        
        
        if (messageDict && ![messageDict isEqual:[NSNull null]]) {
            
        NSMutableDictionary *messageDetailDict=[messageDict valueForKey:@"message"];
            
            
            if (messageDetailDict && ![messageDetailDict isEqual:[NSNull null]]) {
                NSTimeInterval interval=(long long)[messageDetailDict valueForKey:@"1401346788032"];
                NSDate *messageDate = [NSDate dateWithTimeIntervalSince1970:interval];
                
                JSQMessage *message=[[JSQMessage alloc]initWithText:[messageDetailDict valueForKey:@"body"] sender:[messageDetailDict valueForKey:@"user_name"] date:messageDate];
                NSLog(@"%@",message);
